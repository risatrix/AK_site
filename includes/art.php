<h1>"Art"</h1>
	 <p>In my free time, I perform at the <a href="http://dionysium.com/" class="fancybox">Dionysium</a>, Austin's original intellectual variety show. That's a lot of what I do, dream up creative ways to communicate ancient history...</p> 
	
	<div class="pics">  		
		<div class="cell">						
			<a href="img/dionysium_slides/01.png" class="Ifancybox folio_link" rel="group1" title="Slides for a Dionysium lecture - Photoshop. 1)Illustrating the Iliad." >
				<img src="img/dionysium_slides/01.png" alt="An illustration for Homer's Iliad" / class="thumb">
				<div class="title-slider">Presentation Slideshow</div>
			</a>
			<a href="img/dionysium_slides/02.png" class="Ifancybox" rel="group1" title="2)Phryne unrobed" ></a>
			<a href="img/dionysium_slides/03.png" class="Ifancybox" rel="group1" title="3)Obvious, but important." ></a>
			<a href="img/dionysium_slides/04.png" class="Ifancybox" rel="group1" title="4)Yup, the ancient Greeks knew about 'em." ></a>
			<a href="img/dionysium_slides/05.png" class="Ifancybox" rel="group1" title="5)Uma Thurman made an awesome Medusa." ></a>
			<a href="img/dionysium_slides/06.png" class="Ifancybox" rel="group1" title="6)Possibly the best thing..." ></a>
			<a href="img/dionysium_slides/07.png" class="Ifancybox" rel="group1" title="7)Go, Aristophanes!" ></a>         
		</div>
		<div class="cell">	
			<a href="img/ekk-poster.png" rel="group2" class="Ifancybox folio_link" title="Glossy poster for an ancient Greek play I tranlsated - don't ask. Photoshop/Acrobat">
				<img src="img/ekk-poster.png" class="thumb"/>
				<div class="title-slider">Performance Ads</div>
			</a>	
			<a href="img/ekk-flyer.png" class="Ifancybox" rel="group2" title="Flyer for the staged reading, made for easy copying."></a>
		</div>
		<div class="cell">	
			<a href="img/wntw.png" class="Ifancybox folio_link" title="Photoshop Fun - What Would It Look Like if Plato Met Stacey and Clinton?">
				<img src="img/wntw.png" class="thumb"/>
				<div class="title-slider">Plato Meets <em>WNTW</em></div>
			</a>                                            
		</div>
	</div><!--end .pics-->    	                                                                                  
	   
	<p>As an Austin creative I'm required to show you some photographs I've taken - my camera also does video, so expect some amateur movies soon!
			 
	<div class="pics">
		<div class="cell">
			<a href="img/photos/R2.JPG" class="Ifancybox folio_link" title="Fromt the Dionysium Robot Show. By all accounts the awesomest picture I've ever taken.">
				<img src ="img/photos/R2.JPG" class="thumb">
				<div class="title-slider">Pictures of Robots</div>
			</a>
		</div>
		<div class="cell">	
		  	<a href="img/photos/moontower-wanda-sykes.png" class="Ifancybox folio_link" title="Wanda Sykes from when I was 'Press' at Moontower Comedy - totally worth getting yelled at by the theater manager!">
		  		<img src ="img/photos/moontower-wanda-sykes.png" class="thumb">
		  		<div class="title-slider">Moontower Comedy</div>
		  	</a>
		</div>
		<div class="cell">  	
			<a href="img/chopped-large.png" class="Ifancybox folio_link" title="Me, with camera.">
				<img src ="img/chopped-thumb.png" class="thumb"><div class="title-slider">Self-portrait</div>
			</a>
		</div>	              
    </div><!--end .pics--> 